...
| The `api.Lib` struct handed back by `lib.New` | **library entry point** |
| One function field of `api.Lib`, such as `IsPresent` | **lib function** |
| A function filling one field of a struct contract, taking a pointer to the struct that carries the state | **factory** |
| The struct a factory takes a pointer to — an output struct in `/sandbox/lib/` | **carrier** |
