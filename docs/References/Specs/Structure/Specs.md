...
- The document must let a reader construct the project from scratch — including creating `sandbox/`, `sandbox/contracts/`, `sandbox/config/`, `sandbox/lib/`, `examples/libraryExamples/`, and `docs/` — using the schema it maps and the specifications it names.
...
3. **One `##` section per top-level directory** (`Root`, `/sandbox/`, `/examples/libraryExamples/`, `/docs/`), each with a `File | Description | Spec` table (or nested subsection tables), separated by `---`. Directories nested inside a top-level directory (e.g. `/sandbox/contracts/`, `/sandbox/lib/`) are documented as subsections within their parent's `##` section.
