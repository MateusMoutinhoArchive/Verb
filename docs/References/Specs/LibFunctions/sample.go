...
// It shows both halves of a lib function: the factory file under
// sandbox/lib/publicfunctions/, and the New constructor in
// sandbox/lib/new.go that assigns its return value.
...
// ---------------------------------------------------------------
// The constructor below lives in sandbox/lib/new.go, package lib, where
// every factory of publicfunctions is called exactly once.
// ---------------------------------------------------------------
...
func New(multiplier int) api.Lib {
	...
	l.ExampleFunction = publicfunctions.ExampleFunctionFactory(&l)
	l.NewExampleObject = publicfunctions.NewExampleObjectFactory(&l)
	return l
}
