...
There is no internal type and no method set. `sandbox/lib/` holds **factories**: functions that take a pointer to the api struct and return a closure for one of its function fields; the caller assigns the result.

```go
// sandbox/lib/publicfunctions/lib.go
...
The closure captures `l`, so `l.Args`/`l.Used` are read and mutated when `IsPresent` is *called*, not when the factory ran. That is what carries the parsed state into behavior the caller can hold, while the factories in `sandbox/lib/` stay the library's own business.
...
Binding a method into a field would work in Go, but the project does not do it: one shape for filling struct contracts means one place to look for completeness — the `New` at the bottom of the file. The rule is binding; see the [Factories](/docs/References/Specs/Factories/Specs.md) specification.
